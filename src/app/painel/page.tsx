...
export default async function PainelPage() {
  ...
  const empresa =
    user.role === "construtora"
      ? (await db.select().from(construtoras).where(eq(construtoras.ownerUserId, user.id)).limit(1))[0]
      : (await db.select().from(imobiliarias).where(eq(imobiliarias.ownerUserId, user.id)).limit(1))[0];
  ...
  return (
    <section className="mx-auto max-w-6xl px-6 py-12 md:py-16 min-h-[60vh]">
      ...
      {/* Onboarding pendente — bloqueia uso */}
      {!podeOperar && (
      ...
      {/* Aviso intermediário pra quem fez onboarding mas ainda não foi aprovado */}
              ...
              {operacoesRecentes.map((op) => (
                <li key={op.id} className="border-b border-border last:border-0">
                  ...
                  >
                    ...
                    <div className="col-span-5 md:col-span-3 text-right md:text-right font-mono tabular text-sm text-fg">
