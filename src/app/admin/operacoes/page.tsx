...
export default async function AdminOperacoesPage({ searchParams }: Search) {
  ...
  const [stats, operacoes] = await Promise.all([
    getAdminOperacoesStatBoxes(),
    getAllOperacoes({ status: statusFilter, from, to, q, minValor, maxValor }),
  ]);
  ...
  return (
    <AdminShell active="/admin/operacoes" userName={admin.nome}>
      ...
      {/* Busca + range de valor */}
      <form
        method="get"
        className="rounded-2xl border border-border bg-bg-elev p-4 md:p-5 mb-4 grid grid-cols-1 md:grid-cols-4 gap-3"
      >
        ...
        <div className="md:col-span-4 flex items-center gap-2">
          ...
          {(q || params.minValor || params.maxValor) && (
            <Link
